...
def soliton(X,t):
    ...
    z2 = h2 * h1**2
    z = np.sqrt(z1 / z2)
    return  h1 + (h2 - h1) * 1.0/(np.cosh(xi/2.0 * z)**2)
...
class mom_at_t0:
    def uOfXT(self,X,t):
        h = soliton(X,t)-bathymetry_function([0]) #max(soliton(X,t)-bathymetry_function(X),0.0)
        h11 = h1-bathymetry_function([0]) #max(soliton(X,t)-bathymetry_function(X),0.0)
        return D*(h-h11)
...
def getDBC_heta(x,flag):
    None
    #if x[0]==0:
    #    return lambda x,t: (h1-bathymetry_function([0]))**2
    #None
    #if x[0]==0 or x[0]==L[0]:
