...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def getResidual(self,u,r):
        ...
        if(self.comm.rank()==0):
            outputList=[ self.u[0].femSpace.elementMaps.psi,
            ...
            #import json
            #for idx,item in enumerate(outputList):
            #      fileString = 'residualInputs/'+str(idx) + '.txt'
            #      #numpy.savetxt(fileString,item) 

            #      try:
            #      #  #fileObj = open(fileString,'wb')
            #      #  numpy.savetxt(fileString,item) 
            #        item.tofile(fileString,sep="\n")
            #      except (AttributeError):
            #        print "This didn't work ",idx
            #        pass
            #      #if(fileObj):
            #      #  fileObj.close()
        #if (self.comm.rank()==0):
